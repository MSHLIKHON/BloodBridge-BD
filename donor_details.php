<?php
/** File purpose: Donor Details handles the corresponding BloodBridge BD web workflow. */
...
if (in_array($viewer['role'],['seeker','donor'],true)) {
...
$canViewPrivate = can_access_health(db(),$viewer,(int)$donor['id']);
$canViewContact = $canViewPrivate || $acceptedConnection || $viewer['role']==='admin';
$canVerify = $viewer['role']==='hospital' && $canViewPrivate;
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            bb_transaction(db(),function()use($viewer,$id,$status,$verified,$notes):void {
                $pdo=db();bb_one($pdo,'SELECT id FROM users WHERE id=? FOR UPDATE',[(int)$id]);
                if(!can_access_health($pdo,$viewer,(int)$id)) throw new DomainException('Screening consent was revoked.');
                if($status==='Eligible' && (!$verified || $notes==='')) throw new DomainException('Eligible status needs confirmed clinical screening and a reference note.');
...
<div class="detail-grid">
    <section class="content-card">
        ...
        <dl class="detail-list">
            ...
            <div><dt>Next interval date</dt><dd><?= $eligibleDate ? e(date('d M Y', strtotime($eligibleDate))) : 'Requires screening' ?></dd></div>
            ...
            <div><dt>Contact</dt><dd><?= $canViewContact ? e($donor['phone']) : 'Shared after request acceptance' ?></dd></div>
        </dl>
    </section>
    ...
    <section class="content-card private-medical-card">
        ...
        <p><a href="health_records.php?user_id=<?= (int)$donor['id'] ?>">Open private test reports</a></p>
        <dl class="detail-list">
