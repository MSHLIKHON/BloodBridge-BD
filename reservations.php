...
require_role(['seeker', 'hospital', 'admin']);
$pdo = db();
$user = current_user();
$errors = [];
$statusFilter = trim((string) ($_GET['status'] ?? ''));
$allowedReservationStatuses = ['Pending', 'Approved', 'Collected', 'Rejected', 'Cancelled'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        try {
            ...
            $canReview = $user['role'] === 'admin' || ($user['role'] === 'hospital' && (int) $user['hospital_id'] === (int) $reservation['hospital_id']);
            ...
            if ($action === 'approve') {
                ...
                if ($reservation['status'] === 'Approved') $pdo->prepare('UPDATE blood_inventory SET reserved_units = GREATEST(reserved_units - ?, 0) WHERE id = ?')->execute([(int) $reservation['units'], (int) $inventory['id']]);
...
$where = [];
$params = [];
if ($user['role'] === 'seeker') { $where[] = 'br.seeker_id = ?'; $params[] = (int) $user['id']; }
...
if (in_array($statusFilter, $allowedReservationStatuses, true)) {
    $where[] = 'br.status = ?';
    $params[] = $statusFilter;
}
